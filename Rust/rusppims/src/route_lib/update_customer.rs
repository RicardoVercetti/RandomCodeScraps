...
impl UpdateCustomerResponse {
    fn new(resp_code: &str, unique_id: &str, old_unique_id: &str) -> Self {
        UpdateCustomerResponse { data: UpdateCustomerResponseData { 
...
pub async fn update_customer_handler(
    ...
    Json(payload): Json<UpdateCustomerRequest>) -> Json<UpdateCustomerResponse> {
        ...
        let customer_in_question = {
            let found_customer = find_n_get_mut_customer_info(&mut *customer_list, &payload.data.update_customer.unique_id);
            found_customer
        };
        ...
        match customer_in_question {
            Some(customer) => {
                ...
                res = UpdateCustomerResponse::new(
                    "000",
            ...
            None => {
                ...
                println!("No customer found for ID: {}", &request_customer_data.unique_id);
                res = UpdateCustomerResponse::new(
                    "400",
                    &request_customer_data.unique_id, 
                    "NA"
